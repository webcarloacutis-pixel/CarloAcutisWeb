...
El operador Python exige `verify-full`; Prisma 6 exige `sslmode=require&sslaccept=strict` y `sslcert` cuando el endpoint necesita una CA explícita. Nunca se acepta un certificado inválido. La CA pública verificada del runtime está versionada en `carlo-back/certs/supabase-prod-ca-2021.crt`. Prisma configura `sslcert` con esa ruta desde el directorio de trabajo `carlo-back`; en Render es `/opt/render/project/src/carlo-back/certs/supabase-prod-ca-2021.crt`. No requiere Secret Files. El operador de mantenimiento conserva su configuración privada independiente.
