...
**Única corrección manual:** Render → acutis-api-staging → Environment → Secret Files → supabase-prod-ca-2021.crt → sustituir Contents por el archivo PEM completo verificado, incluyendo las líneas delimitadoras inicial y final. Mantener el nombre y DATABASE_URL. Guardar un Secret File puede desencadenar un despliegue: esta publicación no lo guarda ni despliega; esa acción queda en manos del propietario.
