...
export function chatErrorKey(error:unknown) {
  ...
  const keys:Partial<Record<AiErrorCode,string>>={AI_DISABLED:'chat.disabled',AI_RATE_LIMIT:'chat.rateLimit',AI_QUOTA_EXCEEDED:'chat.quota',AI_TIMEOUT:'chat.timeout',AI_RESPONSE_INVALID:'chat.invalid',AI_PERSISTENCE_FAILED:'chat.persistence'}
...
export async function postAiChat(params:{message:string;lang?:string;sessionId?:string;requestId?:string},signal?:AbortSignal):Promise<{answer:string}> {
