...
export function parseCliArgs(args: readonly string[], cwd: string): CliOptions {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (arg === "--kind" && ["prayer", "verse"].includes(value)) options.kind = value as ContentType;
...
export function executionConfig(env: Environment): { model: string; budget: BudgetConfig; timeoutMs: number; retries: 0 | 1 } {
  ...
  return {
    model, timeoutMs, retries: Number(retries) as 0 | 1,
    budget: {
