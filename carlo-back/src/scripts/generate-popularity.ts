...
import { inputHash, METHODOLOGY_VERSION, PopularityError, type PopularityContent } from "../popularity/domain";
import { assertDatabaseTarget, executionConfig, parseCliArgs } from "../popularity/cli-config";
import { createContentSource, prayerContent, prayerSelect, readVerses } from "../popularity/content-source";
import { generateBatch, SpendBudget } from "../popularity/runner";
...
export async function main(args = process.argv.slice(2)): Promise<void> {
  ...
  const budget = configuration ? new SpendBudget(configuration.budget) : null;
  ...
  try {
    if (options.kind === "prayer" || options.execute) {
    ...
    if (!options.execute) {
      process.stdout.write(JSON.stringify({
        mode: "dry-run", methodologyVersion: METHODOLOGY_VERSION, model,
    ...
    process.stdout.write(JSON.stringify({ mode: "execute", ...result, nextCursor }, null, 2) + "\n");
    if (result.results.some(item => ["failed", "budget_exhausted", "changed"].includes(item.status))) process.exitCode = 1;
  } finally {
